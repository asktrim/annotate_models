  ...
  class << self
    ...
    def get_foreign_key_info(klass, options={})
      ...
      sorted_foreign_keys.each do |fk|
        ...
        if options[:show_foreign_key_names]
          fk_display_name = fk.name
        else
          fk_display_name = ""
        end

        if options[:format_markdown]
          fk_info << sprintf("# * `%s`%s:\n#     * **`%s`**\n", fk_display_name, constraints_info.blank? ? '' : " (_#{constraints_info}_)", ref_info)
        else
          fk_info << sprintf("#  %-#{max_size}.#{max_size}s %s %s", fk_display_name, "(#{ref_info})", constraints_info).rstrip + "\n"
        end
      end
